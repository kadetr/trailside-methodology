...
Called by f-start when snapshot.nextAction is non-empty. Receives the
human's workId decision as a parameter — no ASK.
...
- Human decision on whether to continue has already been made by f-start.
...
- If retainedWorkId is null, work.workId remains null and the caller
  (f-work-run) will handle workId selection. session.phase defaults to 1
  for fresh starts (no snapshot to read from).
