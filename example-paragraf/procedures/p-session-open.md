...
- `workId: string` — the workId to open (provided by f-start)

## Preconditions

- `state/current.yaml` session.* and snapshot.* are clean (reset by
  p-session-continue or fresh schema).
- `workId` is set — f-start has already confirmed the workId via ASK.
- Work-pool entry exists for this workId.

## Sequence

```
1. SET session.sessionId [timestamp YYMMDD-HHMM]
